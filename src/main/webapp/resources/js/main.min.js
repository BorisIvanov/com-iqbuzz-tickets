...
function lockUI(){$(".app-row button, #sale, #reservation, #person, #find-reservation").prop("disabled",!0),$(".nav-pills li").off()}function unlockUI(){$(".app-row button, #sale, #reservation, #person, #find-reservation").prop("disabled",!1),$(".nav-pills li").on("click",seanceClick)}function seanceClick(){$(".nav-pills li").removeClass("active"),$(this).addClass("active"),$.cookie("activeSeance",$("a",this).text()),seanceGet()}function seanceGet(){lockUI(),$(".app-row button").prop("disabled",!0).removeClass("btn-primary btn-success btn-info"),$.get(res.url.ticket.list+$(".nav-pills li.active").text().trim(),function(e){for(var t=0;t<e.length;t++)$("button[data-row='"+e[t].row+"'][data-seat='"+e[t].seat+"']").addClass(0==e[t].type?"btn-success":"btn-info");unlockUI()})}function costAdd(e){costSum+=parseFloat(e),$("#costSum").val(costSum)}function costReset(){costSum=0,$("#costSum").val("")}function seatClick(){var e=$(this),t=e.data("status");t?"selected"==t&&(e.data("status","").removeClass("btn-primary"),costAdd(e.data("cost"))):(e.data("status","selected").addClass("btn-primary"),costAdd(e.data("cost")))}function getSeatSelected(){var e=$(".app-row .btn-primary");if(0==e.length)return null;var t=$(".nav-pills li.active").text().trim(),a={ticketList:[]};return $.each(e,function(){var e=$(this);a.ticketList.push({seat:e.data("seat"),row:e.data("row"),seance:t,cost:e.data("cost")})}),a}function saleClick(){var e=getSeatSelected();e&&$.postJSON(res.url.ticket.sale,e.ticketList,function(){$(".app-row .btn-primary").removeClass("btn-primary").addClass("btn-success"),costReset()})}function getPerson(){var e=$("#person").val().trim();return e?$(".person-group").removeClass("has-error"):$(".person-group").addClass("has-error"),e}function reservationClick(){var e=getPerson();if(e){var t=getSeatSelected();t&&(t.person=e,$.postJSON(res.url.ticket.reservation,t,function(){$(".app-row .btn-primary").removeClass("btn-primary").addClass("btn-info"),$("#person").val(""),costReset()}))}}function reservationFind(){var e=getPerson();e&&(lockUI(),$.get(res.url.ticket.reservation+e,function(t){reservationInfo={person:e,seances:[]};for(var a={seance:"",cost:0,tickets:[]},s=0;s<t.length;s++){var n=t[s];a.seance==n.seance?(a.cost=a.cost+n.cost,a.tickets.push({seat:n.seat,row:n.row})):(a={seance:n.seance,cost:0,tickets:[{seat:n.seat,row:n.row}]},reservationInfo.seances.push(a))}reservationTemplateUpdate(),unlockUI()}))}function reservationTemplateUpdate(){var e=Handlebars.compile($("#reservation-info-template").html());$("#reservation-info").text("").append(e(reservationInfo)),$("#reservation-sale button").on("click",reservationSale)}function reservationSale(){var e=$(this).data("seance"),t=$(this).data("person");$.postJSON(res.url.ticket.reservation_sale,{person:t,seance:e},function(){for(var t=reservationInfo.seances,a=0;a<t.length;a++)if(t[a].seance==e){t.splice(a,1);break}reservationTemplateUpdate(),seanceGet()})}var refreshTimeoutDelay=6e4,costSum=0,reservationInfo=null;jQuery.ajaxSetup({error:unlockUI}),$.postJSON=function(e,t,a){return lockUI(),jQuery.ajax({headers:{"Content-Type":"application/json"},type:"POST",url:e,data:JSON.stringify(t),dataType:"json",success:function(e){e.error?($(".modal-body p").text(e.error),$("#modal").modal("show")):a(e),unlockUI()},error:unlockUI})},$(document).ready(function(){$(".app-row .btn-default").on("click",seatClick),$(".nav-pills li").on("click",seanceClick),$("#sale").on("click",saleClick),$("#reservation").on("click",reservationClick),$("#find-reservation").on("click",reservationFind),seanceGet(),setInterval(seanceGet,refreshTimeoutDelay)});